...
public class WriteScores : MonoBehaviour
{
    ...
    public void WriteTheScores()
    {
        ...
        {
            fields = line.Split(',');
            if (Convert.ToInt32(fields[0]) < Fruit.cutFruit && currentScoreWritten == false)
            {
                Debug.Log("Here");
                writer.Write(Fruit.cutFruit);
                writer.Write(',');
                writer.Write(ReadScores.thePlayerName);
                writer.Write("\n");
               // Summary.text += ReadScores.thePlayerName + " - " + Fruit.cutFruit + "\n";
                writer.Write(line + "\n");
